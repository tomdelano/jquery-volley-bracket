...
($ => {
  ...
  function semiFinalist<TTeam, TScore, TMData, TUData>(
    match: Match<TTeam, TScore, TMData, TUData>
  ): boolean {
    const el = match.el;
    const winner = el.find(".jqteam.win");
    winner.append('<div class="bubble sf">SF</div>');
    return true;
  }
  ...
  function prepareWinners<TTeam, TScore, TMData, TUData>(
    ...
    const roundCount =
      Math.log(teams.length * 2) / Math.log(2) - roundCountAdjust;
  ...
  function prepareLosers<TTeam, TScore, TMData, TUData>(
    ...
    const roundCount =
      Math.log(teamPairsCount * 2) / Math.log(2) - 1 - roundCountAdjust;
      ...
      for (let n = 0; n < subRounds; n += 1) {
        ...
        for (let m = 0; m < matchCount; m += 1) {
          ...
          const isConsolationMatch =
            r === roundCount - 1 && skipGrandFinalComeback;
          const isLastLooserRound =
            r === roundCount - 1 &&
            n === subRounds - 1 &&
            switchToSingleEliminationInSF;
          const match = round.addMatch(
            teamCb,
            Option.of(
              isLastLooserRound
                ? semiFinalist
                : isConsolationMatch ? consolationBubbles : null
            )
          );
          match.setAlignCb(
          ...
          if (isConsolationMatch || isLastLooserRound) {
  ...
  function prepareFinalsInSkipDeInSF<TTeam, TScore, TMData, TUData>(
    ...
    // create SF matches
    const sfMatch1 = sfRound.addMatch(
      () => [
        { source: () => winners0.winner() },
        { source: () => losers0.winner() }
      ],
      Option.empty()
    );
    sfMatch1.setAlignCb(tC => {
      const height: number = winners.el.height() / opts.init.teams.length;
      sfMatch1.el.css({ height });
      const top: number = topmargin;
      tC.css({ top });
    });

    const sfMatch2 = sfRound.addMatch(
      () => [
        { source: () => winners1.winner() },
        { source: () => losers1.winner() }
      ],
      Option.empty()
    );
    sfMatch2.setAlignCb(tC => {
      const height: number = winners.el.height() / opts.init.teams.length;
      sfMatch2.el.css({ height });
      const top: number = topmargin;
      tC.css({ top });
    });
    ...
    finalMatch.setAlignCb(tC => {
      const height = winners.el.height() / opts.init.teams.length;
      finalMatch.el.css({ height });

      const top: number = topmargin + finalMatch.el.height() / 2;
      tC.css({ top });
    });
    ...
    if (!opts.skipConsolationRound) {
      ...
      consol.setAlignCb(tC => {
        ...
        const top = topmargin + consol.el.height();
        tC.css({ top });
      });
    }
    resizeContainer();
  ...
  class Round<TTeam, TScore, TMData, TUData> {
    ...
    private margin = this.isLastRound
      ? this.opts.roundMargin + 20
      : this.opts.roundMargin;
  ...
  function countRounds(
    ...
    if (isSingleElimination) {
      ...
      return Math.max(
        2,
        (Math.log(teamPairsCount * 2) / Math.log(2) - 1) * 2 - 1
      ); // DE - grand finals
  ...
  class Match<TTeam, TScore, TMData, TUData> {
    ...
    constructor(
      ...
      if (opts.showMatchUserData) {
        const matchinfo = this.matchUserData
          ? this.matchUserData
          : opts.userDataEmptyString; // handle undefined
        this.matchInfoCon = $(
          // matchinfo
    ...
    public render() {
      ...
      if (this.alignCb !== null) {
        ...
        if (this.opts.showMatchUserData) {
          const topInfo =
            parseFloat(topTeamCon.slice(0, -2)) +
            this.opts.userDataTopCorrection;
          this.matchInfoCon.css({ top: topInfo }); // matchinfo
        }
      }
  ...
  const JqueryBracket = <TTeam, TScore, TMData, TUData>(
    ...
    if (opts.headersForSubBrackets) {
      // FIXME: if it is false it leads to an ugly looser bracket positioning
      $('<div class="losersheader"><p>Losers</p></div>').prependTo(lEl);
      $('<div class="finalsheader"><p>Finals</p></div>').prependTo(fEl);
      $(
        '<div style="height: ' +
          (w.el.height() + l.el.height() + 10) +
          'px;"><div>'
      ).appendTo(fEl);
    }
  ...
  function parseOptions<TTeam, TScore, TMData, TUData>(
    ...
    const opts: Options<TTeam, TScore, TMData, TUData> = {
      ...
      switchToSingleEliminationInSF:
        input.switchToSingleEliminationInSF || false,
